...
public class AppOpsSummary extends InstrumentedFragment {
    ...
    private boolean shouldProvideFakeContacts() {
       int clicked = Settings.System.getInt(getContext().getContentResolver(), "provide_fake_contacts", 2);
       if (clicked == 1)
               return true;
       else
               return false;
	...
	menu.findItem(R.id.privacy_guard_fake_contacts).setChecked(shouldProvideFakeContacts());
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.show_system_apps:
                ...
                resetAdapter();
                return true;
            case R.id.privacy_guard_fake_contacts:
                final String nameForSettingsSystem = "provide_fake_contacts";
                item.setChecked(!item.isChecked());
                int i = ((item.isChecked()) ? 1 : 0);
                Settings.System.putInt(getContext().getContentResolver(), nameForSettingsSystem, i);
                resetAdapter();
                return true;
